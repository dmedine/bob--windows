...
static void bob_resonance(t_bob *x, t_float resonance)
{
    if (resonance < 0)
        resonance = 0;
    else if (resonance > 10)
        resonance = 10;
    x->x_params.p_resonance = resonance;
}
...
static void bob_print(t_bob *x)
{
    ...
    post("resonance %f", x->x_params.p_resonance);
...
{
    x->x_sr = sp[0]->s_sr;
    dsp_add(bob_perform, 5, x, sp[0]->s_vec, sp[1]->s_vec,
        sp[2]->s_vec, sp[0]->s_n);
}
...
void bob_tilde_setup(void)
{
    ...
    class_addmethod(bob_class, (t_method)bob_saturation, gensym("saturation"),
        A_FLOAT, 0);
    class_addmethod(bob_class, (t_method)bob_resonance, gensym("resonance"),
        A_FLOAT, 0);
